...
app.get("/", function (req, res, next) {
  console.log("I come from country/list");
  req.getConnection(function (error, conn) {
    ...
    conn.query(
      "SELECT * FROM node_countries ORDER BY id DESC",
...
app.post("/add", function (req, res, next) {
  ...
  if (!errors) {
    ...
    req.getConnection(function (error, conn) {
      conn.query(
        "INSERT INTO node_countries SET ?",
        country,
        function (err, result) {
          //if(err) throw err
          if (err) {
...
app.get("/edit/(:id)", function (req, res, next) {
  req.getConnection(function (error, conn) {
    conn.query(
      "SELECT * FROM node_countries WHERE id = " + req.params.id,
      function (err, rows, fields) {
        if (err) throw err;
...
app.put("/edit/(:id)", function (req, res, next) {
  ...
  if (!errors) {
    ...
    req.getConnection(function (error, conn) {
      conn.query(
        "UPDATE node_countries SET ? WHERE id = " + req.params.id,
        country,
        function (err, result) {
          //if(err) throw err
          if (err) {
...
app.delete("/delete/(:id)", function (req, res, next) {
  ...
  req.getConnection(function (error, conn) {
    conn.query(
      "DELETE FROM node_countries WHERE id = " + req.params.id,
      country,
      function (err, result) {
        //if(err) throw err
        if (err) {
          ...
          req.flash(
            "success",
            "Country deleted successfully! id = " + req.params.id
          );
          // redirect to countries list page
          res.redirect("/countries");
        }
